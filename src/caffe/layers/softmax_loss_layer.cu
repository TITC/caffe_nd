...
__global__ void SoftmaxWeightedLossForwardGPU(const int nthreads,
          const Dtype* prob_data, const Dtype* label, Dtype* loss,const float* loss_weights,
          const int num, const int dim, const int spatial_dim,
          const bool has_ignore_label_, const int ignore_label_,
          Dtype* counts) {
  CUDA_KERNEL_LOOP(index, nthreads) {
    ...
    if (has_ignore_label_ && label_value == ignore_label_) {
      ...
      loss[index] = -log(max(prob_data[n * dim + label_value * spatial_dim + s],
                      Dtype(FLT_MIN)))*loss_weights[label_value];
      counts[index] = 1;
    }
  }
}
...
void SoftmaxWithLossLayer<Dtype>::Forward_gpu(
 ...
 //      CAFFE_CUDA_NUM_THREADS>>>(nthreads, prob_data, label, loss_data,lossWeights,
...
__global__ void SoftmaxWeightedLossBackwardGPU(const int nthreads, const Dtype* top,
          ...
          const int ignore_label_, Dtype* counts,const float* lossWeights) {
...
void SoftmaxWithLossLayer<Dtype>::Backward_gpu(const vector<Blob<Dtype>*>& top,
  ...
  if (propagate_down[0]) {
	...
	if(rand()%200==0)
	LOG(INFO)<<"weight loss 0 ="<<lossWeights_cpu[0]<<"  weight loss 1 ="<<lossWeights_cpu[1]<<"  weight loss 2 ="<<lossWeights_cpu[2];
